  ...
  export default {
    ...
    methods: {
      ...
      loadPrevClick(event) {//阅读的手动切换章节（通过点击位置判断）
        ...
        if (this.loadCurrentPage == $("#magazine").turn("pages")) {
          if (parseInt(clientX / width * 100) > 78) {//如果点击的区域大于屏幕宽度的78%就doSomething....
            this.loadPrev(1);
          } else if (parseInt(clientX / width * 100) < 23) {
            $("#magazine").turn("previous");
          }
      ...
      getLink(sourceId) {
        ...
        let url2 = `${this.GLOBAL.kuaYu}http://chapter2.zhuishushenqi.com/toc/${sourceId}?view=chapters`;
        ...
        axios.get(txtUrl).then((response) => {
          ...
          if (response.status == 200) {
            ...
            if (data.ok) {
              ...
              //console.log(newArr2);
              let newArr3 = [];
              let hangAll = parseInt(($('#book-read').height() - 80) / (this.rangeValue * 1.5));//几行？总高度/ 一行的高度
      ...
      loadPrev(num) {
        ...
        if (this.page < 0) {
          ...
          this.loadCurrentPage = $("#magazine").turn("pages");
          this.allLoaded = true;//判断是否全部加载完毕
        } else {
        ...
        class getNovel {
          ...
          getNovel2() {
            return new Promise((resolve, reject) => {
              let url1 = `${selfVue.GLOBAL.kuaYu}http://api.zhuishushenqi.com/toc?view=summary&book=${this.sourceId}`;
          ...
          getText2() {//http://chapter2.zhuishushenqi.com
            return new Promise((resolve, reject) => {
              ...
              let url3 = `${selfVue.GLOBAL.kuaYu}http://chapter2.zhuishushenqi.com/chapter/${encodeURIComponent(chapters[selfVue.page].link)}?k=2124b73d7e2e1945&t=1468223717`;
